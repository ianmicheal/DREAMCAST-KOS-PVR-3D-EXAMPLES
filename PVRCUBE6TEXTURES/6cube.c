...
/********************************************************************************************/
/* KallistiOS Direct PVR API:2.0  PVR TEXTURE CUBE WITH 6 TEXTURES: ZOOM AND
 * ROTATION, V2.0 */
/********************************************************************************************/
/********************************************************************************************/
/* Name:     cube6.c */
/* Title:    PVR TEXTURE CUBE WITH 6 TEXTURES: ZOOM AND ROTATION, V2.0 Kos
 * Example          */
 ...
 * 3D           */
/* And commented so anyone who knows OpenGL can use the DIRECT NO LAYER KOSPVR
 * API.         */
/* History: version 2 */
/********************************************************************************************/
/********************************************************************************************/
...
/*  Ian Michael
 * This needed alignment as it worked on DC emulators. They all allow
 non-aligned
 * memory reads, which is an instant crash on the SH4 CPU and the Dreamcast, as
 * they do not allow this. I fixed this with major printf and add2line
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    if (fabs(xspeed) < 0.0001f)
      xspeed = 0;
    if (fabs(yspeed) < 0.0001f)
      yspeed = 0;
